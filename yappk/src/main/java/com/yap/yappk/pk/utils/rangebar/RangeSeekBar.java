...
import static com.yap.yappk.pk.utils.rangebar.SeekBar.INDICATOR_ALWAYS_HIDE;
import static com.yap.yappk.pk.utils.rangebar.SeekBar.INDICATOR_ALWAYS_SHOW;
...
public class RangeSeekBar extends View {
    ...
    //Scale mode：number(Arrange according to the actual proportion of the numbers) other Evenly arranged
    ...
    //Progress of the minimum interval remaining
    ...
    private void initProgressBitmap() {
        if (progressBitmap == null) {
            progressBitmap = RangeBarUtils.drawableToBitmap(getContext(), progressWidth, progressHeight, progressDrawableId);
        }
        if (progressDefaultBitmap == null) {
            progressDefaultBitmap = RangeBarUtils.drawableToBitmap(getContext(), progressWidth, progressHeight, progressDefaultDrawableId);
        }
    }
    ...
    private void initStepsBitmap() {
        ...
        if (stepsBitmaps.isEmpty()) {
            Bitmap bitmap = RangeBarUtils.drawableToBitmap(getContext(), (int) stepsWidth, (int) stepsHeight, stepsDrawableId);
    ...
    private void initAttrs(AttributeSet attrs) {
        try {
            ...
            progressHeight = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_progress_height, RangeBarUtils.dp2px(getContext(), 2));
            ...
            tickMarkTextMargin = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_margin, RangeBarUtils.dp2px(getContext(), 7));
            tickMarkTextSize = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_size, RangeBarUtils.dp2px(getContext(), 12));
    ...
    protected int getTickMarkRawHeight() {
        if (tickMarkTextArray != null && tickMarkTextArray.length > 0) {
            return tickMarkTextMargin + RangeBarUtils.measureText(String.valueOf(tickMarkTextArray[0]), tickMarkTextSize).height() + 3;
        }
        return 0;
    }
        ...
        if (tickMarkTextArray != null) {
            ...
            for (int i = 0; i < tickMarkTextArray.length; i++) {
                ...
                if (tickMarkMode == TRICK_MARK_MODE_OTHER) {
                    ...
                    float num = RangeBarUtils.parseFloat(text2Draw);
                    SeekBarState[] states = getRangeSeekBarState();
                    if (RangeBarUtils.compareFloat(num, states[0].value) != -1 && RangeBarUtils.compareFloat(num, states[1].value) != 1 && (seekBarMode == SEEKBAR_MODE_RANGE)) {
    ...
    protected void onDrawProgressBar(Canvas canvas, Paint paint) {
        ...
        if (RangeBarUtils.verifyBitmap(progressDefaultBitmap)) {
        ...
        if (RangeBarUtils.verifyBitmap(progressBitmap)) {
    ...
    public SeekBarState[] getRangeSeekBarState() {
        ...
        if (RangeBarUtils.compareFloat(leftSeekBarState.value, minProgress) == 0) {
            leftSeekBarState.isMin = true;
        } else if (RangeBarUtils.compareFloat(leftSeekBarState.value, maxProgress) == 0) {
        ...
        if (seekBarMode == SEEKBAR_MODE_RANGE) {
            ...
            if (RangeBarUtils.compareFloat(rightSB.currPercent, minProgress) == 0) {
                rightSeekBarState.isMin = true;
            } else if (RangeBarUtils.compareFloat(rightSB.currPercent, maxProgress) == 0) {
    ...
    public void setStepsDrawable(List<Integer> stepsDrawableIds) {
        ...
        for (int i = 0; i < stepsDrawableIds.size(); i++) {
            stepsBitmaps.add(RangeBarUtils.drawableToBitmap(getContext(), (int) stepsWidth, (int) stepsHeight, stepsDrawableIds.get(i)));
        }
